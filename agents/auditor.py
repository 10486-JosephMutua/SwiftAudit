...
class AuditorAgent:
    ...
    def _generate_summary_with_agent(
        ...
        """Generate executive summary directly via LLM for a clean text response."""
        
        critical = sum(1 for f in findings if str(f.severity).upper() == "CRITICAL")
        high     = sum(1 for f in findings if str(f.severity).upper() == "HIGH")
        
        system_prompt = (
            "You are a senior security consultant. Write a professional executive summary "
            "for a security audit report. Focus on the risk grade and key findings. "
            "Write only 2-3 concise paragraphs. Use plain text, no markdown, no JSON."
        )
        ...
        user_prompt = (
            ...
            f"Grade: {risk_grade}\n"
            f"Findings: {len(findings)} total ({critical} Critical, {high} High)."
        )

        try:
            # Using direct call_llm is much safer for text generation than an agent
            from utils.llm_client import call_llm
            summary = call_llm(
                system_prompt=system_prompt,
                user_prompt=user_prompt,
                label="auditor_summary_text"
            )
            return summary.strip()
        except Exception as e:
            logger.error(f"[AUDITOR] Summary generation failed: {e}")
            return f"SwiftAudit completed security analysis of {repo_url}. Risk score: {risk_score}/100 (Grade: {risk_grade})."
